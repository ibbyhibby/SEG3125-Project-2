...
const Nintendo = () => {
  ...
  const gameNames = ['Super Luigi Bros', 'Super Mario Bros', 'Duck Hunt', 'Mario Bros 1/3', 'Super Mario 2', 'Mario Anniversary', 'Super Mario Bros 3', 'Super Mario Mix', 'Super Mario Duel', 'Super Mario Unlimited']

  const initialGames = Array(10).fill().map((_, i) => ({
    img: `/nintendo/nintendo${i + 1}.png`,
    name: gameNames[i] || `Nintendo Game ${i + 1}`,
    generation: `Generation ${((i % 3) + 1)}`,
    difficulty: ['Easy', 'Medium', 'Hard'][i % 3],
    price: Math.floor(Math.random() * 41) + 20,
    link: '/Nintendo-Product'
  }));
  ...
  const handleCardClick = (game) => {
    navigate('/Nintendo-Product', { state: { game } });
  }
